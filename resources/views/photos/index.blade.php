    ...
    @foreach($photos as $key => $value)
        <tr>
            ...
            <td>
                ...
                {{ Form::open(array('url' => 'photos/' . $value->id, 'class' => 'pull-right')) }}
                    {{ Form::hidden('_method', 'DELETE') }}
                    {{ Form::submit('Delete this Nerd', array('class' => 'btn btn-warning')) }}
                {{ Form::close() }}

                <!-- show the nerd (uses the show method found at GET /photos/{id} -->
                <a class="btn btn-small btn-success" href="{{ URL::to('photos/' . $value->id) }}">Show this Nerd</a>

                <!-- edit this nerd (uses the edit method found at GET /photos/{id}/edit -->
                <a class="btn btn-small btn-info" href="{{ URL::to('photos/' . $value->id . '/edit') }}">Edit this Nerd</a>
